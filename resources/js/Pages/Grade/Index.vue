  ...
  <authenticated-layout>
    <section class="py-5 mt-5">
      <div class="container d-flex gap-4 flex-column justify-content-start align-items-start">
        ...
        <Link class="btn btn-success" href="/dashboard/grades/create">اضافة صف</Link>
            ...
            <tbody>
              <tr v-for="grade in grades" :key="grade.id">
                ...
                <td class="d-flex gap-2 justify-content-center">
                  <Link class="btn btn-sm btn-warning" :href="`/dashboard/grades/${grade.id}/edit`">تعديل</Link>
...
export default {
  ...
  watch: {
    search: debounce(function (newSearch, oldSearch) {
      this.$inertia.get('/dashboard/grades', {'search': newSearch}, {preserveState: true, replace: true})
    }, 500),
    filter: debounce(function (value) {
      this.$inertia.get('/dashboard/grades', {'filter': value}, {preserveState: true, replace: true})
    }, 300)
  },
  methods: {
    filterSchools () {
    ...
    destroy(id) {
      if (confirm('هل انت متأكد ؟')) {
        this.$inertia.delete(`/dashboard/grades/${id}`)
      }
    }
  }
}
